...
pub fn setup_rtc_wakeup(syscfg: &mut SYSCFG, exti: &mut EXTI, rtc: &mut RTC) {
    ...
    rtc.wutr.modify(|_, w| unsafe { w.wut().bits(0x0) });
    // pg 795 todo: Is this what we want? 000 sets RTC/16, but I don't know what that
    // or any of the terms mean.
    ...
    // todo do we need to use raw registers? Can't find this in the hal crate.
    // rtc.cr.modify(|_, w| w.wutf().bit(false));

    // set timer period?
    rtc.wutr.modify(|_, w| unsafe { w.bits(2047) });
}
